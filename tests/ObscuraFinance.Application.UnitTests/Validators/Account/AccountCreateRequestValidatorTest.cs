...
namespace ObscuraFinance.Application.UnitTest.Validators.Account
{
    ...
    {
        private readonly AccountCreateRequestValidator _validator;

        public AccountCreateRequestValidatorTest()
        {
            _validator = new AccountCreateRequestValidator();
        }

        private static AccountCreateRequest CreateValidRequest()
        {
        ...
        [Fact]
        public void InitialBalance_Should_HaveError_When_BelowMinimum()
        {
            // Arrange
            var request = CreateValidRequest();
            request.InitialBalance = AccountConstraints.MinimumInitialBalance - 1;
            ...
            result.ShouldHaveValidationErrorFor(x => x.InitialBalance)
                .WithErrorMessage($"Initial balance must be between {AccountConstraints.MinimumInitialBalance} and {AccountConstraints.MaximumInitialBalance}.");
        }

        [Fact]
        public void InitialBalance_Should_HaveError_When_AboveMaximum()
        {
            // Arrange
            var request = CreateValidRequest();
            request.InitialBalance = AccountConstraints.MaximumInitialBalance + 1;
        ...
        [Fact]
        public void InitialBalance_Should_NotHaveError_When_WithinRange()
        {
            // Arrange
            var request = CreateValidRequest();
